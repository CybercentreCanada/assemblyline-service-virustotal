        ...
        if report_type == "file":
            ...
            if request.get_param('analyze_relationship'):
                ...
                for relationship in [k for k, v in response.get('relationships', {}).items() if v.get('data')]:
                    ...
                    title_text = relationship.title()\
                        .replace('_', " ").replace('Ip', 'IP').replace('Url', 'URL').replace('Itw', 'ITW')
                ...
                for relationship, data in response.get('relationships', {}).items():
                    ...
                    interim_section = ResultSection(title_text=title_text, body=", ".join(body),
                                                    body_format=BODY_FORMAT.TEXT, parent=relationships_section,
                                                    tags=tags, auto_collapse=True)
